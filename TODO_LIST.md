...
TACHE 2 : Modélisation indépendante de la bourse. 
   ...
   [X] Le temps de calcul du script est très long, sûrement à cause de la sélection des dates étudiées. Je ne peux même pas savoir car le script ne termine jamais.
      [X] C'est le premier problème à résoudre : essayer d'éxecuter le script en local et observer ce qui bloque le temps d'éxecution, et réparer le problème. Si une feature prend beaucoup trop de temps de calcul, alors il faut la shunter pour le moment et la désactiver / remplacer par une version plus simple.
      - Le bloc `markov_switching_2_regimes_skew_t` (calibration multi-start la plus coûteuse) est désormais désactivé par défaut dans la pipeline CLI, réactivable via `--inclure-markov-skew-t`; la calibration interne a aussi été allégée (`n_starts=4`, `maxiter=150`) pour conserver un mode activable plus rapide.
   [X] Il semblerait que la stratégie skew t asymetrique soit la plus intéressante. Il faudrait trouver un paramétrage qui puisse fitter encore mieux la courbe historique, notamment en augmentant la présence des queues épaisses négatives. Il pourrait donc être intérressant de diminuer le paramètre mu pour cette fonction ou de jouer un peu avec les paramètres.
   - Au final on garde celle qui fit le mieux sans biaiser l'analyse.
   [X] Il faut maintenant pouvoir "exporter" la fonction identifiée afin de pouvoir l'incorporer au moteur de simulation au dessus. Il faut donc un fichier python "generer_trajectoires_bourse.py" que l'on pourra appeler en lui fournissant la date de départ, date de fin, nombre de MC, graine initiale. En sortie, elle fournira une liste de séries temporelles (une grande matrice) qui contiendra, pour chaque mois, pour chaque tirage MC, les variations (en facteur multiplicatif, donc -2% donne 0.98, +0.3% donne 1.003).
   - Module standalone ajouté dans `src/modelisation_macro/bourse/` avec `generer_trajectoires_bourse.py` + `parametres_bourse.py` (skew-t calibrée et exportable).
      [X] Il faudra sûrement 2 fichiers, avec un principal qui appelle la méthode choisie. La fonction utilisatrice de ces scripts ne sait pas quelle est la modélisation de la bourse, et ça ne l'intéresse pas, elle n'est que consommatrice des résultats.
      [X] Comme il faudra juste pouvoir copier / coller un ou deux fichiers dans le moteur de simulation complet : il faudra être malin sur la stratégie, comme par exemple un fichier principal qui appelle un fichier de paramétrage (lui-même généré par les scripts actuels contenant les informations de la stratégie identifiée et choisie).
      [X] Donc les scripts ici présents génèrent un paramétrage qui pourra être utilisé par generer_trajectoires_bourse.py, de manière à ce que ces nouveaux fichiers soient standalone pour générer les sorties.
      - Script d'export ajouté : `scripts/export_parametres_bourse.py` (recalibre et régénère `parametres_bourse.py`).
      [] Faire en sorte que la fenêtre d'identification commence au plus tard en 1990 (donc pas possible de commencer en 2009).
   [X] Il faut un script qui permette de tester la génération (qui réplique le comportement qu'aurait le moteur de simulation, donc les entrées qu'elle fournirait). Ce script sera utilisé pour appeler toutes les modélisations, pas uniquement la bourse. Il faut donc qu'il puisse être modulaire pour ajouter toutes les modélisations de la manière que ferait le moteur de simulation.
   - Script `scripts/test_generation_moteur.py` créé avec registre modulaire de générateurs (V1: `bourse`).
      [X] Il faut que ce script de test fasse également un fichier html de sortie qui ressemble à celui qui est généré par la pipeline_identification_univariee. Ce fichier doit présenter, pour chaque variable (au début uniquement la bourse), un graphe plotly (toujours thème sombre interactif) toutes les trajectoires, avec la moyenne et la médiane dans la même couleur. Les graphes devront être utiles pour représenter les infos des variables. Pour la bourse on souhaite observer l'intégrale des variations pour chaque trajectoire.
      - Rapport HTML sombre interactif généré avec trajectoires cumulées, moyenne et médiane en même couleur (style différencié).
      [] Dans le HTML final d'export de simulation de test pour le moteur concernant la bourse, il faudrait pouvoir comparer à l'intégrale des variations historiques en affichant la moyenne et la médiane, comme ça a été fait pour le HTML précédent (comme ici on peut générer des périodes indépendantes de l'historique, il faut afficher uniquement la période historique qui recouvre celle simulée, qui peut être vide). 
      [] Il faudrait également comparer la distribution de probabilités des variations comparée à l'historique sur un autre graphique dessous. Afficher la densité de probabilité, non pas avec un histogramme mais une densité (kde plot) sur un paragraphe séparé et pas un subplot du précédent.
      [] Mettre une erreur dans le script de génération si la date initiale de génération (donc la valeur initiale de la bourse) n'est pas dans la plage historique de valeur, où l'on n'aurait pas de valeur initiale à fournir.
