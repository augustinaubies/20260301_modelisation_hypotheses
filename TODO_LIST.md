...
TACHE 2 : Modélisation indépendante de la bourse. 
   ...
   [] Il faut maintenant pouvoir "exporter" la fonction identifiée afin de pouvoir l'incorporer au moteur de simulation au dessus. Il faut donc un fichier python "generer_trajectoires_bourse.py" que l'on pourra appeler en lui fournissant la date de départ, date de fin, nombre de MC, graine initiale. En sortie, elle fournira une liste de séries temporelles (une grande matrice) qui contiendra, pour chaque mois, pour chaque tirage MC, les variations (en facteur multiplicatif, donc -2% donne 0.98, +0.3% donne 1.003).
      [] Il faudra sûrement 2 fichiers, avec un principal qui appelle la méthode choisie. La fonction utilisatrice de ces scripts ne sait pas quelle est la modélisation de la bourse, et ça ne l'intéresse pas, elle n'est que consommatrice des résultats.
      [] Comme il faudra juste pouvoir copier / coller un ou deux fichiers dans le moteur de simulation complet : il faudra être malin sur la stratégie, comme par exemple un fichier principal qui appelle un fichier de paramétrage (lui-même généré par les scripts actuels contenant les informations de la stratégie identifiée et choisie).
      [] Donc les scripts ici présents génèrent un paramétrage qui pourra être utilisé par generer_trajectoires_bourse.py, de manière à ce que ces nouveaux fichiers soient standalone pour générer les sorties.
   [] Il faut un script qui permette de tester la génération (qui réplique le comportement qu'aurait le moteur de simulation, donc les entrées qu'elle fournirait). Ce script sera utilisé pour appeler toutes les modélisations, pas uniquement la bourse. Il faut donc qu'il puisse être modulaire pour ajouter toutes les modélisations de la manière que ferait le moteur de simulation.
      [] Il faut que ce script de test fasse également un fichier html de sortie qui ressemble à celui qui est généré par la pipeline_identification_univariee. Ce fichier doit présenter, pour chaque variable (au début uniquement la bourse), un graphe plotly (toujours thème sombre interactif) toutes les trajectoires, avec la moyenne et la médiane dans la même couleur. Les graphes devront être utiles pour représenter les infos des variables. Pour la bourse on souhaite observer l'intégrale des variations pour chaque trajectoire.
