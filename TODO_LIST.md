...
TACHE 2 : Modélisation indépendante de la bourse. 
   ...
   [] Le temps de calcul du script est très long, sûrement à cause de la sélection des dates étudiées. Je ne peux même pas savoir car le script ne termine jamais.
      [] C'est le premier problème à résoudre : essayer d'éxecuter le script en local et observer ce qui bloque le temps d'éxecution, et réparer le problème. Si une feature prend beaucoup trop de temps de calcul, alors il faut la shunter pour le moment et la désactiver / remplacer par une version plus simple.
   [] Il semblerait que la markov switching skew mette trop en avant les phases de marché haussières et pas assez les phases baissières. Il est trop optimiste et ne peut donc pas être utilisé en tant que modélisation représentative du marché. Légèrement modifier la stratégie markov switching skew t pour corriger ce problème. Essayer de faire ça le plus proprement possible sans bricolage ni paramétrage manuel, dans la mesure du possible.
