...
TACHE 2 : Modélisation indépendante de la bourse. 
   ...
   [] Il semblerait qu'il y ait toujours un problème. Effetivement on l'observe sur la valeur de ks stat que les lois ne sont pas si proches de l'historique, alors que c'est leur unique but : minimiser l'erreur.
      [] Vérifier que les hyperparamètres des lois sont correctement calculer pour fitter l'historique sur la bonne fenêtre (depuis la date choisie).
      [] La "gaussienne théorique (fit historique)" est très éloigné de la gaussienne historique (bonne fenêtre), il y a donc clairement un problème : sa moyenne et écart type ne correspondent pas aux données du tableau "Diagnostic de calibration des lois" des données historique. Le but de cette courbe est justement de répliquer fidèlement la distribution de probabilité, c'est son unique but. Mettre à jour la méthode d'identification si nécessaire.
